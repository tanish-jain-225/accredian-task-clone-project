...
export default function MobileMenu({ isOpen, onClose }) {
  return (
    <>
      ...
      {/* Slide-in panel */}
      <div
        className={`fixed top-0 right-0 z-[60] h-full w-[85%] max-w-sm bg-white shadow-2xl flex flex-col transform transition-transform duration-500 ease-in-out ${
      ...
      >
        ...
        {/* Header */}
        <div className="flex items-center justify-between px-8 py-8 border-b border-gray-50 relative z-10">
          <div>
            <span className="text-2xl font-black text-gray-950 tracking-tighter">
              accredian<span className="text-brand-600">.</span>
            </span>
            <p className="text-[8px] text-gray-400 font-black tracking-[0.3em] uppercase">enterprise</p>
          </div>
          <button
            onClick={onClose}
            className="w-12 h-12 flex items-center justify-center rounded-2xl bg-gray-50 text-gray-950 hover:bg-gray-100 transition-colors"
        ...
        <nav className="flex-1 px-4 py-10 space-y-2 relative z-10 overflow-y-auto">
          {navLinks.map((link) => (
            <a
              ...
              className="group flex items-center justify-between px-6 py-4 rounded-2xl text-gray-950 font-black text-sm uppercase tracking-widest hover:bg-brand-50 hover:text-brand-600 transition-all duration-300"
            >
              {link.label}
        ...
        <div className="px-8 pb-12 relative z-10">
          <div className="p-6 bg-brand-50 rounded-3xl border border-brand-100 mb-6">
